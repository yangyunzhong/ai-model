...
`chenjiaqi` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/chenjiaqi/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/chenjiaqi/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --mine` to show tasks assigned
  to them.
...
python3 ./.trellis/scripts/task.py archive 00-join-chenjiaqi
```
