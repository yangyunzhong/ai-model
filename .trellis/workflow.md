...
**B Create a task** — any implementation / code change / build / refactor work, and any PM/UI prototype request that will create or modify a runnable mock, mock data, prototype notes, draft API, draft data model, handoff package, or verification record. Entry sequence: (1) `python ./.trellis/scripts/task.py create "<title>"` to create the task (status=planning, breadcrumb switches to [workflow-state:planning] for brainstorm + jsonl phase guidance) → (2) for PM/UI prototype work, load `product-prototype` and `.trellis/spec/prototype/`; for other unclear requirements, load `trellis-brainstorm`; keep `prd.md` current → (3) once prd is done and jsonl is curated, run `task.py start <task-dir>` to enter [workflow-state:in_progress] for the implementation skeleton. **"It looks small" is NOT grounds for downgrading B to A or C**.
...
If the active task is PM/UI prototype work, load `product-prototype` and `.trellis/spec/prototype/`, then iterate on `prd.md` with design-first planning. Otherwise load the `trellis-brainstorm` skill and iterate on prd.md with the user.
...
If the user asks for a new PM/UI prototype that is unrelated to the current active task, do not hide it inside the current task. Create or switch to a dedicated Trellis prototype task, then continue with `product-prototype` and `.trellis/spec/prototype/`.
...
Skip when `python ./.trellis/scripts/task.py current --source` already points to a task that matches the current work. If the user asks for a new PM/UI prototype unrelated to the active task, create or switch to a dedicated prototype task instead of recording it under the unrelated task.

#### 1.1 Requirement exploration `[required · repeatable]`

Load the right planning skill and explore requirements interactively with the user:

- PM/UI prototype work: load `product-prototype` and `.trellis/spec/prototype/`, then keep the task `prd.md` focused on design-first planning.
- Other unclear requirements: load the `trellis-brainstorm` skill per the skill's guidance.

The planning skill will guide you to:
