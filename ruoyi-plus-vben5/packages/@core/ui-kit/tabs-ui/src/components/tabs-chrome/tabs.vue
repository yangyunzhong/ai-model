...
function canCloseTab(tab: TabConfig) {
  return !tab.affixTab && tab.closable && tabsView.value.length > 1;
}

function showDisabledCloseIcon(tab: TabConfig) {
  return props.showDisabledCloseIcon && !canCloseTab(tab);
}
      ...
      >
        <VbenContextMenu :handler-data="tab" :menus="contextMenus" :modal="false" item-class="pr-6">
          ...
          >
            <!-- close-icon -->
            <X
              v-show="canCloseTab(tab) || showDisabledCloseIcon(tab)"
              :class="{
                'tabs-chrome__close-icon--disabled pointer-events-none cursor-default opacity-40':
                  showDisabledCloseIcon(tab),
              }"
              class="tabs-chrome__close-icon mt-0.5 size-3 cursor-pointer rounded-full stroke-accent-foreground/80 text-accent-foreground/80 transition-all group-[.is-active]:text-accent-foreground hover:bg-accent hover:stroke-accent-foreground"
              @click.stop="() => emit('close', tab.key)"
            />
            <Pin
              v-show="
                !props.showDisabledCloseIcon &&
                tab.affixTab &&
                tabsView.length > 1 &&
                tab.closable
              "
