...
import { Breadcrumb, ContentPageTabs, Preferences } from '../widgets';
...
// Layout themes: top header, left sidebar, and secondary sidebar can differ.
...
const isPlatformRailNav = computed(() => {
  return preferences.navigation.styleType === 'platform-rail';
});

const useContentPageTabs = computed(() => !preferences.breadcrumb.enable);

const layoutTabbarVisible = computed(() => {
  return preferences.tabbar.enable || useContentPageTabs.value;
});

const layoutTabbarHeight = computed(() => {
  return useContentPageTabs.value ? 44 : preferences.tabbar.height;
});

// The top header owns only first-level navigation when rail mode is not active.
const showHeaderNav = computed(() => {
  return (
    !isMobile.value &&
    !isPlatformRailNav.value &&
    (isHeaderNav.value ||
      isMixedNav.value ||
...
<template>
  <!--
    Layout structure:
    1. Top header bar occupies the full first row.
    2. Left sidebar and right content area both live below the header.
    3. VbenAdminLayout is only the slot assembler; the visual regions are defined below.
  -->
  <VbenAdminLayout
    ...
    :sidebar-style-type="preferences.navigation.styleType"
    :sidebar-theme="sidebarTheme"
    :sidebar-theme-sub="sidebarThemeSub"
    :sidebar-width="preferences.sidebar.width"
    :side-collapse-width="preferences.sidebar.collapseWidth"
    :tabbar-enable="layoutTabbarVisible"
    :tabbar-height="layoutTabbarHeight"
    ...
    "
  >
    <!-- Top header brand: logo icon + system title -->
    <template #logo>
      <div
        v-if="preferences.logo.enable"
        class="vben-header-brand"
        @click="clickLogo"
      >
        <VbenLogo
        ...
        />
        <slot name="logo-text">
          <span class="vben-header-brand__text">
            {{ preferences.app.name }}
          </span>
        </slot>
      </div>
    </template>
    <!-- First row: top navigation bar -->
    <template #header>
    ...
    <!-- Second row / left column: primary sidebar navigation -->
    <template #menu>
      <LayoutMenu
        ...
        :rounded="isMenuRounded"
        :style-type="preferences.navigation.styleType"
        :theme="sidebarTheme"
        mode="vertical"
        @open="handleMenuOpen"
        @select="handleMenuSelect"
    ...
    <!-- Second row / left column extra area: split or mixed navigation submenu -->
    <template #side-extra>
      <LayoutExtraMenu
        :accordion="preferences.navigation.accordion"
    ...
    <template #tabbar>
      <ContentPageTabs v-if="useContentPageTabs" />
      <LayoutTabbar
        v-else-if="preferences.tabbar.enable"
        :show-icon="preferences.tabbar.showIcon"
        :theme="theme"
      />
    </template>

    <!-- Second row / right column: page content area -->
    <template #content>
      <LayoutContent />
    </template>
...
.vben-header-brand :deep(.vben-logo__link) {
  padding-inline: 22px 12px;
}

.vben-header-brand__text {
  display: inline-flex;
  align-items: center;
  min-width: 0;
  color: hsl(var(--header-foreground, 0 0% 100%));
  font-size: 24px;
  font-weight: 600;
  line-height: 24px;
  letter-spacing: 0;
